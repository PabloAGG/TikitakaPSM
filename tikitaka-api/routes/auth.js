...
router.post('/register', validateRegister, async (req, res) => {
    try {
        ...
        const { email, password, username, team_id, full_name } = req.body;
        ...
        const [result] = await db.execute(
            'INSERT INTO users (email, password, username, team_id, full_name) VALUES (?, ?, ?, ?, ?)',
            [email, hashedPassword, username, team_id, full_name || username]
        );
...
router.post('/login', validateLogin, async (req, res) => {
    try {
        ...
        const [users] = await db.execute(`
            SELECT u.id, u.email, u.password, u.username, u.full_name, u.profile_image,
                   t.name as team_name, t.logo_url as team_logo
            FROM users u 
            JOIN teams t ON u.team_id = t.id 
            WHERE u.email = ?
