            ...
            try {
                ...
                if (postIds.length > 0) {
                    const [userLikes] = await db.execute(
                        ...
                        [decoded.userId, ...postIds]
                    );
                    
                    const [userFavorites] = await db.execute(
                        `SELECT post_id FROM favorites WHERE user_id = ? AND post_id IN (${postIds.map(() => '?').join(',')})`,
                        [decoded.userId, ...postIds]
                    );
                    ...
                    posts.forEach(post => {
                        ...
                        post.is_favorited = favoritePosts.has(post.id);
                    });
                }
            } catch (error) {
                // Si hay error con el token, continuar sin datos de usuario
                posts.forEach(post => {
                    post.is_liked = false;
...
router.put('/:id', authenticateToken, validatePost, async (req, res) => {
    try {
        ...
        // Actualizar post
        await db.execute(
            'UPDATE posts SET content = ?, team_id = ?, is_draft = ? WHERE id = ?',
            [content, team_id, is_draft, postId]
        );
